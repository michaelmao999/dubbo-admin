...
public class ConsumersPageHandler implements PageHandler {
    ...
    public Page handle(URL url) {
        ...
        if (service != null && service.length() > 0) {
            ...
            List<org.apache.dubbo.common.URL> consumers = RegistryContainer.getInstance().getConsumersByService(service);
            if (consumers != null && consumers.size() > 0) {
                for (org.apache.dubbo.common.URL u : consumers) {
            ...
            List<List<String>> rows = new ArrayList<List<String>>();
            List<org.apache.dubbo.common.URL> consumers = RegistryContainer.getInstance().getConsumersByApplication(application);
            if (consumers != null && consumers.size() > 0) {
                for (org.apache.dubbo.common.URL u : consumers) {
                    List<String> row = new ArrayList<String>();
                    String s = u.toFullString();
