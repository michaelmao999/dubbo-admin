...
import org.apache.dubbo.monitor.simple.common.Constants;
    ...
    public Page handle(URL url) {
        ...
        if (hosts != null && hosts.size() > 0) {
            for (String host : hosts) {
                ...
                List<org.apache.dubbo.common.URL> providers = RegistryContainer.getInstance().getProvidersByHost(host);
                List<org.apache.dubbo.common.URL> consumers = RegistryContainer.getInstance().getConsumersByHost(host);

                if (providers != null && providers.size() > 0
                        || consumers != null && consumers.size() > 0) {
                    org.apache.dubbo.common.URL provider = (providers != null && providers.size() > 0 ? providers.iterator().next() : consumers.iterator().next());
